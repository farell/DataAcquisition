...
{
    public class CsvAcquisitorFeiLong : Acquisitor
    {
        private Dictionary<string, SensorInitVal> initValue;
        private string stampFileName;
        private string initFileName;
        private DateTime dateTime;
        private Dictionary<string, MidValueFilter> filterDictionary;
        public CsvAcquisitorFeiLong(string initFile)
        {
            this.initFileName = initFile;
            this.filterDictionary = new Dictionary<string, MidValueFilter>();
            ...
            try
            {
                ...
                while (line != null)
                {
                    ...
                    for (int i = 0; i < type.Length; i++)
                    {
                        ...
                        string key = property[i] + "-" + type[i];

                        MidValueFilter filter = null;

                        if (this.filterDictionary.ContainsKey(key))
                        {
                            filter = this.filterDictionary[key];
                        }
                        else
                        {
                            filter = new MidValueFilter(7);
                            this.filterDictionary.Add(key, filter);
                            Console.WriteLine("Create Filter: " + key);
                        }

                        double dat = Double.Parse(datas[i]);
                        DataValue dv = new DataValue();
                        dv.SensorId = longId;
                        dv.TimeStamp = cur_date;
                        dv.Value = filter.GetFilteredValue(dat).ToString();
                        ...
                        if (id[i] == "T1" || id[i] == "T3" || id[i] == "T4" || id[i] == "T6")
                        {
                            ...
                            key = property[i] + "-" + type[i] + "-deflection";
                            ...
                            dv2.Value = filter.GetFilteredValue(deflection).ToString();
        ...
        private void GetAccAndVibr(string fileName, ref Queue<DataValue> dataQueue)
        {
            uint count = 0;
            uint totalRecords = 0;
        ...
        public override void AcquisitData(ref Queue<DataValue> dataQueue, string prefix)
        {
            ...
            GetAccAndVibr(accFilename, ref dataQueue);
            GetAccAndVibr(vibFilename, ref dataQueue);
        }
    }
}
